...
<!--learn point estimates of network weights-->
...
- Parameters: network weights $\mathbf{w}$ and biases $b$. <!-- http://edwardlib.org/tutorials/bayesian-neural-network -->

<!-- - In the Bayesian approach we put **priors** on our latent variables to reflect our uncertainty in their values  --> <!--$p(\mathbf{w})$ and $p(\mathbf{b})$.--> 
...
<!--Diagram of an NN? Assume you know what this is. -->
...
<!---mention something about program traces--->
...
<!-- A universal inference engine (give some examples).  Hide details to make probabilistic modeling more accessible to the working programmer. But can scale poorly. -->
...
<!---Mention Inference as a first class citizen--->
<!---can implement recent models like GANs, VAEs more easily--->

- Allows data subsampling for greater **scalability**

<!--- ADVI in pymc3 and Stan can do this--->
<!-- - *Stochastic optimisation* by representing only a subgraph of the full model: this allows Stochastic Variational Inference etc. -->
...
- Approximate the posterior $p(\mathbf{w} \vert \mathbf{X, Y})$ with a simpler distribution from a variational family of distributions $q_{\theta}(\mathbf{w})$ parameterised by $\theta$ <!--C Naesseth 2017 -->
<!--Gal 2.1.1 and Dropout preso-->
...
<!--Explain how this enforces Occam's razor, penalises model complexity -->
<!--Explain how this is a lower bound on the evidence: log p(x) = KL( q(z) || p(z|x) ) + ELBO(q) -->
...
<!-- Need an approximation of  **Expected Log Likelihood** $\int q_{\theta}\text{ log } p(\mathbf{Y} \vert {\mathbf{X, \mathbf{w}}})d\mathbf{w}$ and its **gradient** --> <!--check this--> 

>- **Gradient of ELBO**: estimate using Monte Carlo sampling from the variational posterior $q_{\theta}(\mathbf{w})$ *(score functions, reparameterisation trick)*  <!-- need to clarify that this holds for the case of reparameterization trick-->
...
<!-- read some text about stochastic gradients -->
...
Rezende et al., 2014). The reparameterization trick is applicable to most continuous latent-variable models, and usually provides lower-variance gradient estimates - Duvenaud et al (2017) --> 
...
<!-- what about gradient estimation>-->
...
<!-- Duplicate this slide to bring in results incrementally, n=50 then n=10-->
